...
@click.command()
@click.option('--genomes', required=True, type=click.Path(exists=True, file_okay=False, dir_okay=True),
...
def main(genomes: str, output: str, template: Optional[str]) -> None:
    """
    Proksee Batch: A tool for batch processing of genomes using Proksee (https://proksee.ca/).
    """
    ...
    # Set template to the path to a default Proksee configuration file if no template was provided.
        ...
        template = os.path.join(os.path.dirname(__file__), 'data', 'default_proksee_template.json')
    ...
    for genome in os.listdir(genomes):
        if genome.endswith('.gbk'):
            ...
            genbank_description, genbank_total_size, genbank_number_of_contigs, genbank_gc_content = get_stats_from_genbank(os.path.join(genomes, genome))

            genome_id = genome.rsplit('.', 1)[0]
            genome_files[genome_id] = {"JSON_file": "",
                                    "Proksee_image_file": "",
            ...
            merged_json_file = os.path.join(temp_output, genome.replace('.gbk', '.merged.json'))
