...
def merge_cgview_json_with_template(basic_json_file: str, template_file: str, output_file: str) -> None:
    """
    Merge a basic cgview map in JSON format with a Proksee configuration file in JSON format.
    """
    # Load the basic cgview map
    with open(basic_json_file, 'r') as basic_json:
    ...
    formatting_keys = ['version', 'created', 'updated', 'settings', 'backbone', 'ruler', 
                       'annotation', 'dividers', 'highlighter', 'captions', 'legend']
    ...
    assert len(merged_data['cgview']['captions']) == 1, "The 'captions' list must contain exactly one element."
